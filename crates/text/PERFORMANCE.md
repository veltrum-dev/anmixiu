...
with `cargo bench -p anmixiu-text`. Times are Criterion 95% intervals for
...
Reproduce timing with `cargo bench -p anmixiu-text --bench text -- --noplot`
and allocation data with `cargo bench -p anmixiu-text --bench allocation`.
