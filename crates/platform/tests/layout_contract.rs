...
use anmixiu_platform::{
    Align, Dimension, Edges, FlexDirection, Justify, LayoutEngine, LayoutError, LayoutNode,
    LayoutNodeId, LayoutRequest, LayoutRevisions, LayoutStyle, LayoutViewport, MeasureId,
};
...
fn request(root: &LayoutNode, width: f32, height: f32) -> LayoutRequest<'_> {
    LayoutRequest::new(
        root,
        LayoutViewport::new(Size::new(width, height), 1.0),
...
fn text_measurement_receives_constraints_and_scale() {
    ...
    let request = LayoutRequest::new(
        &root,
        LayoutViewport::new(Size::new(100.0, 80.0), 2.0),
    ...
    assert!(constraints_seen.iter().all(|constraints| {
        constraints.known_width == Some(100.0)
            || constraints.available_width == anmixiu_platform::AvailableLength::Definite(100.0)
    }));
    assert_eq!(
...
fn every_declared_revision_invalidates_layout() {
    ...
    let mut engine = LayoutEngine::new();
    let viewport = LayoutViewport::new(Size::new(100.0, 40.0), 1.0);
    let revisions = [
        LayoutRevisions::new(1, 1, 1),
...
fn resize_and_scale_are_part_of_the_invalidation_key() {
    ...
    let first = engine
        .compute(
            LayoutRequest::new(
                &root,
                LayoutViewport::new(Size::new(320.0, 200.0), 1.0),
                revisions,
            ),
            |_, _| Size::default(),
        )
    ...
    let resized = engine
        .compute(
            LayoutRequest::new(
                &root,
                LayoutViewport::new(Size::new(640.0, 400.0), 1.0),
                revisions,
            ),
            |_, _| Size::default(),
        )
    ...
    let scaled = engine
        .compute(
            LayoutRequest::new(
                &root,
                LayoutViewport::new(Size::new(640.0, 400.0), 2.0),
                revisions,
            ),
            |_, _| Size::default(),
        )
