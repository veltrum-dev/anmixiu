...
- Keep the public surface as ordinary Rust values and chainable builders. Model UI with typed Rust
  builders rather than alternate markup or tag syntax, browser-backed rendering, a parallel GUI
  runtime, a shadow-tree clone, or a global string/tag registry.
...
- Keep dependency direction toward contracts: platform implementations may depend on core contracts,
  but core/reactive/scene/runtime/facade crates must not depend back on winit, wgpu, CoreText, or
...
- `unsafe`/FFI is limited to target-specific modules in `anmixiu-text`. Every unsafe block or impl
  needs an adjacent `// SAFETY:` explanation. Core, reactive, scene, runtime, platform, render,
  facade, macros, and examples remain safe Rust.
...
- Keep logical pixels, scale-adjusted floating coordinates, and integer device pixels distinct.
  Configure wgpu surfaces from winit's exact physical resize dimensions and current scale factor;
  test 1x/2x placement and raster-sensitive changes.
...
- Keep platform differences behind target-specific modules/crates and dispatch with `#[cfg]` (or an
  ...
  Shared contracts must not depend on winit, wgpu, CoreText, DirectWrite, or other
